...
final class InventoryReservationsModule implements ModuleInterface {
    public function name(): string { return 'table-inventory_reservations'; }
    public function table(): string { return 'inventory_reservations'; }
    public function version(): string { return '1.0.0'; }
    /** @return string[] */
    public function dialects(): array { return [ 'mysql', 'postgres' ]; }
    /** @return string[] */
    public function dependencies(): array { return [ 'table-books' ]; }
    ...
    public function uninstall(Database $db, SqlDialect $d): void {
        $view = 'vw_inventory_reservations';
        if ($d->isMysql()) {
            $db->exec("DROP VIEW IF EXISTS `$view`");
        } else {
            $db->exec("DROP VIEW IF EXISTS $view");
    ...
    public function status(Database $db, SqlDialect $d): array {
        ...
        $indexes = [ 'idx_res_book', 'idx_res_order', 'idx_res_status_until' ];
        $fks     = [ 'fk_res_book' ];
        $missingIdx = [];
        $missingFk  = [];
