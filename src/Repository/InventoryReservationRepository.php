...
use BlackCat\Database\Packages\InventoryReservations\Dto\InventoryReservationDto as Dto;
use BlackCat\Database\Packages\InventoryReservations\Mapper\InventoryReservationDtoMapper as RowMapper;
...
final class InventoryReservationRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\InventoryReservations\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\InventoryReservations\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\InventoryReservations\Dto\InventoryReservationDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\InventoryReservations\Dto\InventoryReservationDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\InventoryReservations\Mapper\InventoryReservationDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\InventoryReservations\Dto\InventoryReservationDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\InventoryReservations\Dto\InventoryReservationDto|null {
