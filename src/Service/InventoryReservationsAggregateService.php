 ...
 */
final class InventoryReservationsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private InventoryReservationRepository $inventoryReservationRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'inventory_reservations' => $this->inventoryReservationRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\InventoryReservations\ModuleException("Record {$idStr} not found for locking.");
            }
